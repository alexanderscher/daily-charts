...
from .models import Prospect
from .models import SoundcloudCharts
...
class FetchDB:
    ...
    def get_soundcloud_charts(self):
        session = SessionLocal()
        ...
        try:
            ...
            recent_date = (
                session.query(SoundcloudCharts.date)
                .filter(SoundcloudCharts.date != today_str)
                .order_by(SoundcloudCharts.date.desc())
                .first()
            )
            ...
            if recent_date:
                ...
                charts = (
                    session.query(SoundcloudCharts)
                    .filter(SoundcloudCharts.date == most_recent_date_str)
                    .all()
                )
                ...
                data = [
                    {
                        ...
                        "soundcloud_link": chart.soundcloud_link,
                        "spotify_link": chart.spotify_link,
                        "label": chart.label,
                        "date": chart.date,
                    }
                    for chart in charts
    ...
    def insert_soundcloud_charts(self, data):
        session = SessionLocal()
        pacific_tz = timezone("America/Los_Angeles")
        ...
        try:
            date_exists = session.query(
                exists().where(SoundcloudCharts.date == current_date)
            ).scalar()
            ...
            if not date_exists:
                ...
                for (
                    ...
                    new_chart_entry = SoundcloudCharts(
                        chart=chart,
                        position=position,
                        artist=artist,
                        song=song,
